...
export default function SalesPageClient({
  ...
  const todayCash = dailyPayments
    .filter((p) => p.method === "CASH")
    .reduce((sum, p) => sum + p.amount, 0);
  const todayQris = dailyPayments
  ...
  const handleExport = () => {
    ...
    const rows = dailyPayments.map((p) => [
      ...
      p.order.source === "CUSTOMER" ? "Self Service" : "Kasir/Waiter",
      p.method,
      p.amount.toString(),
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-primary to-green-600 rounded-2xl p-5 text-white">
            ...
            <p className="text-xs mt-2 opacity-70">
              {dailyPayments.length} transaksi
            </p>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            ...
            <p className="text-xs mt-2 text-green-600 font-semibold">
              {dailyPayments.filter((p) => p.method === "CASH").length}{" "}
              transaksi
            </p>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            ...
            <p className="text-2xl font-black text-slate-900">
              {formatCurrency(todayQris)}
            </p>
            <p className="text-xs mt-2 text-blue-600 font-semibold">
              {dailyPayments.filter((p) => p.method === "QRIS").length}{" "}
              transaksi
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-50">
              {(activeTab === "today"
                ? dailyPayments
                : paidOrders.filter((o) => o.payment)
              ).map((item) => {
                const payment =
                  activeTab === "today"
                    ? (item as PaymentWithOrder)
                    : (item as OrderWithItems).payment!;
                const order =
                  activeTab === "today"
                    ? (item as PaymentWithOrder).order
                    : (item as OrderWithItems);
                  ...
                  <tr key={payment.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4">
                      {order.source === "CUSTOMER" ? (
                        <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-orange-100 text-orange-700">
                          Self Service
                        </span>
                      ) : (
